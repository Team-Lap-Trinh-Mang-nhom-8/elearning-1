...
```
# Server baseline(python)
python .\code\python\tcp_tuning.py server 127.0.0.1 9001
...
### Java Benchmark tương đương (client-side), so sánh tuned vs baseline

```powershell
# Biên dịch 2 benchmark Java
javac .\code\java\TcpLatencyBench.java .\code\java\TcpThroughputBench.java

# Latency (client tuned=false) với server tuned (9000) và baseline (9001)
java -cp .\code\java TcpLatencyBench 127.0.0.1 9000 --runs 200 --size 16 --tuned false
java -cp .\code\java TcpLatencyBench 127.0.0.1 9001 --runs 200 --size 16 --tuned false

# Latency (client tuned=true) để thấy khác biệt do client-side
java -cp .\code\java TcpLatencyBench 127.0.0.1 9000 --runs 200 --size 16 --tuned true
java -cp .\code\java TcpLatencyBench 127.0.0.1 9001 --runs 200 --size 16 --tuned true

# Throughput (chunk ~32KB, giống Python)
java -cp .\code\java TcpThroughputBench 127.0.0.1 9000 --bytes 50000000 --chunk 32768 --tuned false
java -cp .\code\java TcpThroughputBench 127.0.0.1 9001 --bytes 50000000 --chunk 32768 --tuned false

# Tuỳ chọn bật tối ưu ở client để so thêm phương diện client
java -cp .\code\java TcpThroughputBench 127.0.0.1 9000 --bytes 50000000 --chunk 32768 --tuned true
```
